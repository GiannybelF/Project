    ...
    if submitted:
        ...
        if errors:
            ...
            try:
                ...
                # Create plot using Streamlit's built-in charting
                st.subheader("Visualization")
                
                # Create plot data
                x_smooth = np.linspace(x0, x_final, 100)
                y_smooth = y0 * np.exp(k * (x_smooth - x0))
                
                # Combine data for plotting
                plot_data = pd.DataFrame({
                    'x': list(df['x']) + list(x_smooth),
                    'Euler Method': list(df['y (Euler)']) + [None] * len(x_smooth),
                    'Analytical Solution': [None] * len(df) + list(y_smooth)
                })
                
                st.line_chart(plot_data.set_index('x'), use_container_width=True)
                ...
                # Plot error using Streamlit's built-in charting
                error_data = pd.DataFrame({
                    'x': df['x'],
                    'Absolute Error': df['Error']
                })
                
                st.line_chart(error_data.set_index('x'), use_container_width=True)
